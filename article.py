...
def words_per_minute(num_words):
    per_min = num_words//300
    if per_min < 60:
...
class Article:
    ...
    def _hydrate_article(self):
        ...
        article = parseFunction(self.parser)
        try:
            self.word_count = len(article.text.split(" "))
            if self.default_parser_used:
                self.full_content = f'<h3>Full article ({self.word_count} words : est. {words_per_minute(self.word_count)}) [Default method used]</h3>' + unidecode(article.prettify())
            else:
                self.full_content = f'<h3>Full article ({self.word_count} words : est. {words_per_minute(self.word_count)})</h3>' + unidecode(article.prettify())
